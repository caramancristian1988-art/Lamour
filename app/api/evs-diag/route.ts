...
export async function GET() {
  ...
  const actions = await evsRaw("GetAWBActions", { datestart: fmt(new Date(Date.now() - 6.5 * 86400e3)), dateend: fmt(new Date()) });
  ...
  out.actionsMineOld = JSON.stringify(parsed ?? "").includes("LUTCKSRKEO0926");
  out.actionsSample = JSON.stringify(parsed ?? "").slice(0, 1200);
