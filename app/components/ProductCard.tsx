...
export default function ProductCard({
  ...
  // A size/quantity is picked via the "Adaugă în coș" popup, not on the card
  // itself, so the raw trailing "1L" / "2 role" etc. baked into the name
  // would just be confusing clutter here — strip it from the title.
    ...
    <div className="group bg-card rounded-2xl border border-border overflow-hidden flex flex-col transition-all duration-300 hover:shadow-xl focus-within:shadow-xl hover:-translate-y-1">
      ...
      <div className="relative aspect-[4/3] flex items-center justify-center bg-white overflow-hidden">
        ...
        <div className="absolute top-2 right-2 sm:top-4 sm:right-4">
          <FavoriteButton
            product={{ slug, name, price, oldPrice: oldPrice ?? null, image: displayImage, rating, reviewCount, badge }}
          />
        </div>
      </div>
